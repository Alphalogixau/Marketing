'use client'

import { useEffect, useRef, useState } from 'react'
import { motion, useInView, useReducedMotion } from 'motion/react'
import { cn } from '@/lib/utils'
import { incidentLifecycle } from '@/lib/content'

/**
 * Guardian's incident lifecycle, animated.
 *
 * A pulse travels the pipeline and each stage lights as it passes. The three
 * stages flagged `emphasis` are the ones that carry the actual argument —
 * the quality gate, "a detection is not an incident", and shadow mode — so
 * they are visually heavier and hold the pulse longer.
 *
 * Deliberately NOT a camera feed with boxes drawn on people. Guardian ships
 * no recognition of any kind and the UI must not imply otherwise
 * (docs/MARKETING_GUARDRAILS.md).
 */
export function IncidentPipeline() {
  const ref = useRef<HTMLDivElement>(null)
  const inView = useInView(ref, { once: false, margin: '-15% 0px -15% 0px' })
  const reduced = useReducedMotion()
  const [active, setActive] = useState(-1)

  useEffect(() => {
    if (!inView || reduced) {
      if (reduced) setActive(incidentLifecycle.length - 1)
      return
    }

    let stage = 0
    let timer: ReturnType<typeof setTimeout>

    const step = () => {
      setActive(stage)
      const current = incidentLifecycle[stage]
      // Emphasis stages hold longer — they're the point of the diagram.
      const hold = current?.emphasis ? 1900 : 900
      stage = (stage + 1) % incidentLifecycle.length
      timer = setTimeout(step, hold)
    }

    timer = setTimeout(step, 250)
    return () => clearTimeout(timer)
  }, [inView, reduced])

  return (
    <div ref={ref} className="relative">
      <ol className="relative space-y-1">
        {/* Spine */}
        <div
          aria-hidden
          className="absolute left-[1.4375rem] top-4 bottom-4 w-px bg-white/10"
        />

        {incidentLifecycle.map((stage, i) => {
          const isActive = i === active
          const isPast = active > i

          return (
            <li key={stage.n} className="relative">
              <div
                className={cn(
                  'flex gap-5 rounded-xl p-4 transition-colors duration-500',
                  isActive && 'bg-white/[0.04]',
                )}
              >
                {/* Node */}
                <div className="relative z-10 shrink-0">
                  <motion.div
                    animate={
                      reduced
                        ? {}
                        : {
                            scale: isActive ? 1 : 0.85,
                            borderColor: isActive
                              ? 'rgb(206 235 89)'
                              : isPast
                                ? 'rgba(206, 235, 89, 0.35)'
                                : 'rgba(255,255,255,0.15)',
                          }
                    }
                    transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
                    className={cn(
                      'grid size-12 place-items-center rounded-full border-2 bg-navy-900 font-mono text-sm tabular-nums',
                      isActive ? 'text-guardian-400' : isPast ? 'text-guardian-400/60' : 'text-navy-400',
                    )}
                  >
                    {String(stage.n).padStart(2, '0')}
                    {isActive && !reduced && (
                      <motion.span
                        aria-hidden
                        className="absolute inset-0 rounded-full border-2 border-guardian-400"
                        initial={{ opacity: 0.7, scale: 1 }}
                        animate={{ opacity: 0, scale: 1.7 }}
                        transition={{ duration: 1.4, repeat: Infinity, ease: 'easeOut' }}
                      />
                    )}
                  </motion.div>
                </div>

                {/* Copy */}
                <div className="min-w-0 pt-1.5">
                  <h3
                    className={cn(
                      'text-base font-semibold transition-colors duration-500',
                      isActive ? 'text-white' : 'text-navy-200',
                    )}
                  >
                    {stage.label}
                    {stage.emphasis && (
                      <span className="ml-2 align-middle text-[0.625rem] font-bold uppercase tracking-widest text-guardian-500">
                        key
                      </span>
                    )}
                  </h3>
                  <motion.p
                    animate={reduced ? {} : { opacity: isActive ? 1 : 0.72 }}
                    transition={{ duration: 0.45 }}
                    className="mt-1 max-w-xl text-sm leading-relaxed text-navy-300"
                  >
                    {stage.detail}
                  </motion.p>
                </div>
              </div>
            </li>
          )
        })}
      </ol>

      <p className="mt-6 pl-4 text-xs text-navy-400">
        The pipeline as built — see{' '}
        <span className="font-mono text-navy-300">GUARDIAN_V0_1_ARCHITECTURE.md §3</span>.
      </p>
    </div>
  )
}
